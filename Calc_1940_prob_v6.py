...
#%% Remove LENS EM from LENS historical and put LENS PI in anomaly space
...
lens_hist_time,lens_hist_u_nat = load_1d_clim_model_concat_ens('LENS_hist',\
                                'u10','ASE',isolate_nat=True) 
lens_hist_time,lens_hist_psl_nat = load_1d_clim_model_concat_ens('LENS_hist',\
                                'psl','ASE',isolate_nat=True) 
    
#also load LENS EM for plotting
lens_em_time,lens_em_u = load_1d_clim_model_em('LENS_hist','u10','ASE')
lens_em_time,lens_em_psl = load_1d_clim_model_em('LENS_hist','psl','ASE')

#put LENS EM in anom space
lens_em_u_anom = lens_em_u - np.mean(lens_em_u.sel(time=slice(1961,1990)))
lens_em_psl_anom = lens_em_psl - np.mean(lens_em_psl.sel(time=slice(1961,1990)))

#remove LENS EM from 1961-1990 in LENS PI so it is in the same anomaly space
lens_em_u_ref = np.mean(lens_em_u.sel(time = slice(1961,1990)))
lens_pi_u_anom = lens_pi_u - np.tile(lens_em_u_ref,len(lens_pi_u))
lens_em_psl_ref = np.mean(lens_em_psl.sel(time = slice(1961,1990)))
lens_pi_psl_anom = lens_pi_psl - np.tile(lens_em_psl_ref,len(lens_pi_psl))

#also remove LENS EM ref value from LENS hist for the calculation with LENS hist including anthro
lens_em_u_ref = np.mean(lens_em_u.sel(time = slice(1961,1990)))
lens_hist_u_anom = lens_hist_u - np.tile(lens_em_u_ref,len(lens_hist_u))
lens_em_psl_ref = np.mean(lens_em_psl.sel(time = slice(1961,1990)))
lens_hist_psl_anom = lens_hist_psl - np.tile(lens_em_psl_ref,len(lens_hist_psl))

#%%
#plot SLP to visualize anomaly ref period
...
if plot_all_slp_data:
    ...
    fig.add_subplot(331)
    plt.plot(lens_em_time,lens_em_psl,label='LENS historical EM raw\nmean 1979-2005=%.2f'%np.mean(lens_em_psl[59:]),color='tab:blue',linestyle='--')
    #also plot ERA
    era_time,era_psl = load_1d_era5('psl',[1979,2005],'ASE')
    plt.plot(era_time,era_psl,color='k',label='ERA5\nmean=%.2f'%np.mean(era_psl))
    plt.legend(loc='upper left')
    plt.annotate('Mean = %.2f'%np.mean(lens_em_psl)+\
                 '\n1961-1990 ref value = %.2f'%lens_em_psl_ref,(0.05,0.05),\
                     xycoords='axes fraction')
    plt.annotate('a',(x,y),xycoords='axes fraction')
    
        
    fig.add_subplot(334)
    plt.plot(lens_em_time,lens_em_psl_anom,label='LENS historical EM anom',color='tab:blue')
    plt.legend(loc='upper left')
    plt.annotate('Mean = %.2f'%np.mean(lens_em_psl_anom),(0.05,0.05),\
                     xycoords='axes fraction')
    ...
    fig.add_subplot(336)
    lens_hist_e1_anom = lens_hist_psl[0:86]-np.tile(lens_em_psl_ref,86)
    ...
    fig.add_subplot(331)
    plt.plot(lens_em_time,lens_em_u,label='LENS historical EM raw\nmean 1979-2005=%.2f'%np.mean(lens_em_u[59:]),color='tab:blue',linestyle='--')
    #also plot ERA
    era_time,era_u = load_1d_era5('u1000',[1979,2005],'ASE')
    plt.plot(era_time,era_u,color='k',label='ERA5\nmean=%.2f'%np.mean(era_u))
    plt.legend(loc='upper left')
    plt.annotate('Mean = %.2f'%np.mean(lens_em_u)+\
                 '\n1961-1990 ref value = %.2f'%lens_em_u_ref,(0.05,0.05),\
                     xycoords='axes fraction')
    plt.annotate('a',(x,y),xycoords='axes fraction')
    
    fig.add_subplot(334)
    plt.plot(lens_em_time,lens_em_u_anom,label='LENS historical EM anom',color='tab:blue')
    plt.legend(loc='upper left')
    plt.annotate('Mean = %.2f'%np.mean(lens_em_u_anom),(0.05,0.05),\
                     xycoords='axes fraction')
    plt.annotate('b',(x,y),xycoords='axes fraction')
    ...
    fig.add_subplot(336)
    lens_hist_e1_anom = lens_hist_u[0:86]-np.tile(lens_em_u_ref,86)
    plt.plot(lens_hist_time[0:86],lens_hist_e1_anom,label='LENS hist E1 anomaly',color='lightsteelblue')
    plt.legend(loc='upper left')
    plt.annotate('LENS hist E1 - LENS EM ref\nMean = %.2f'%np.mean(lens_hist_e1_anom),(0.05,0.05),xycoords='axes fraction')
    # plt.ylim([-10,8])
...
if plot_model_data:
    ...
    if plot_v == 'u10':
        ...
        fig.add_subplot(312)
        plt.plot(lens_em_time,lens_em_u,label='LENS Historical ensemble mean',\
                 color='tab:blue',linewidth=.7)
        plt.legend(loc='lower left')
        
        fig.add_subplot(313)
        plt.plot(lens_hist_time,lens_hist_u_nat,label='LENS Hist - LENS EM',\
        ...
        fig.add_subplot(312)
        plt.plot(lens_em_time,lens_em_psl,label='LENS Historical ensemble mean',\
                 color='tab:blue',linewidth=.7)
        plt.legend(loc='lower left')
    
        fig.add_subplot(313)
        plt.plot(lens_hist_time,lens_hist_psl_nat,label='LENS Hist - LENS EM',\
                 color='lightsteelblue',linewidth=.7)
        plt.legend(loc='upper left')
    
    plt.suptitle('Model ASE shelf break '+plot_v)
...
#have to put LENS EM in anomaly space from 1961-1990 (i.e. LENS EM SLP is ~980hPa)
# u_ref_val = np.mean(lens_em_u[41:71])
lens_em_u_anom = lens_em_u - lens_em_u_ref
# psl_ref_val = np.mean(lens_em_psl[41:71])
lens_em_psl_anom = lens_em_psl - lens_em_psl_ref

em_recon_u10_nat = em_recon_u10 - lens_em_u_anom
em_recon_psl_nat = em_recon_psl - lens_em_psl_anom

#plot 
fig = plt.figure()
fig.add_subplot(211)
plt.plot(recon_time,em_recon_psl,label='Recon EM',color='tab:blue')
plt.plot(lens_em_time,lens_em_psl_anom,label='LENS EM anomaly',color='gray')
...
plt.plot(lens_em_time,lens_em_u_anom,label='LENS EM anomaly',color='gray')
...
fig.add_subplot(211)
plt.plot(recon_time,nat_full_ens_psl[0,:],label = '1st scrambled reconstruction member',color='k')
plt.plot(lens_pi_time,lens_pi_psl_anom,label='LENS PI anomaly',color='gray')
...
fig.add_subplot(212)
plt.plot(recon_time,nat_full_ens_u[0,:],label = '1st scrambled reconstruction member',color='k')
plt.plot(lens_pi_time,lens_pi_u_anom,label='LENS PI anomaly',color='gray')
...
u_events_lens_hist_mean = search_model(lens_hist_u_anom,events,mean_u_thresh)
print('Number of events found using mean threshold:',u_events_lens_hist_mean)
u_events_lens_hist_low = search_model(lens_hist_u_anom,events,low_u_thresh)
print('Number of events found using lower threshold:',u_events_lens_hist_low)
u_events_lens_hist_upp = search_model(lens_hist_u_anom,events,upp_u_thresh)
...
psl_events_lens_hist_mean = search_model(lens_hist_psl_anom,events,mean_psl_thresh)
print('Number of events found using mean threshold:',psl_events_lens_hist_mean)
psl_events_lens_hist_low = search_model(lens_hist_psl_anom,events,low_psl_thresh)
print('Number of events found using lower threshold:',psl_events_lens_hist_low)
psl_events_lens_hist_upp = search_model(lens_hist_psl_anom,events,upp_psl_thresh)
