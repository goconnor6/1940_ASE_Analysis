...
# Also calculate correlation between Nino3.4 index and SLP
recon1_nino_corr, recon1_sig = calc_1d_corr(cesm_recon_psl,nino34_idx)
recon2_nino_corr, recon2_sig = calc_1d_corr(pace_recon_psl,nino34_idx)
...
spec = gridspec.GridSpec(ncols=1, nrows=3,wspace=0.23,
                             hspace=0.2, height_ratios=[1, 1.3, 1])
...
ax0.grid(linestyle = '-', linewidth = '0.3',color = 'gray')  
...
ax1.plot(recon_time, cesm_recon_g_u, color = cesm_recon.g_u10_color,zorder=0,\
         label = 'Natural-prior geostroph.\n'+recon_stats[4].split('\n')[-1], linewidth = lw-.2,linestyle = (0, (5, 1)))
...
ax3.set_ylabel('Nino3.4 Index',fontsize = fs-1,labelpad=3)
...
plt.subplots_adjust(top = 0.98,\
                    bottom = 0.08,right = 0.91,left = 0.12)
#wspace = 0.04,hspace = 0.22,


fig.text(0.125,0.95,'a)',fontsize=fs,weight='bold')
fig.text(0.125,0.655,'b)',fontsize=fs,weight='bold')
fig.text(0.125,0.29,'c)',fontsize=fs,weight='bold')

plt.savefig('Plots/Figure_1_a_c_ASE_timeseries.png', bbox_inches = 'tight',dpi = 600)
